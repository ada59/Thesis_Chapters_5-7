...
load(paste0(myd, "/tt2.RData"))     # For species & sourceII
```
<p>&nbsp;</p>
...
#### **Plot of IA & IB occurrence: **

```{r, echo=TRUE, warning=FALSE, message=FALSE}
head(ContE)
nsites <- gather(ContE, key="Species", value="Presence", -c(1:2))
nsites <- nsites[nsites$Presence==1,]
nsites$SourceII <- tt2$SourceII[match(nsites$Species, rownames(tt2))]
nsites <- nsites[!nsites$SourceII=="NR", ] # TRanslocated species no considered.
sum(nsites$SourceII=="IA") # 37
sum(nsites$SourceII=="IB") # 27
