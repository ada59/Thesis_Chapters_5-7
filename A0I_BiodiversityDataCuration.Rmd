...
dtUP <- read_excel("C:/Users/Usuario/Documents/PHD/ThesisChapterMexico_I/TemporalChange_MexicanFish_C2/Lists/Original/TODO_peces agosto 2010_PGM_Comments.xlsx") # Data file 
```
<p>&nbsp;</p>
#### **Data formatting:**
...
names(tax) <- c("Short", "Short2", "Genus_species", "Synonym", "Update", "Update_comment", "Genus_species (ECF)")
...
tax$Genus_species <- plyr::revalue(tax$Genus_species, c("A   dugesii"="Allotoca dugesii",
                                                ...
                                                "observamos Oreochromis"="Oreochromis spp",
...
***Only species in sites that were visited in both periods are included.
...
# SUBSET 3:
ExcludedII <- anti_join(NDT, NDT67)
ExcludedII <- ExcludedII[!ExcludedII$NativeFound2005 %in% c("nada, es la localidad?", "nada, no era la localidad buscada"),] # removed those that might not have been the same locality
ExcludedII <- ExcludedII[!ExcludedII$SiteName %in% c("rio Marabasco", "laguna El Barril"),] # remove rio Marabasco, laguna el Barril (no data for the native assemblage)

NDT79 <- rbind(NDT67, ExcludedII)

save(NDT79, file=paste0(path_lists, "/NDT79.RData"))
write.csv2(NDT79, file=paste0(path_lists, "/NDT79.csv"))

list67_79 <- anti_join(NDT79, NDT67)
save(list67_79, file=paste0(path_lists, "/list67_79.RData"))
write.csv2(list67_79, file=paste0(path_lists, "/list67_79.csv"))
